...
// si32 additionalInfo; e.g. spell number if type is 1 || 10; tile to attack 
//                      if type is 6

// Implementation of BattleLogic class.
namespace geniusai { namespace battleai {
BattleLogic::BattleLogic(ICallback* cb,
                         CCreatureSet* army1,
                         CCreatureSet* army2,
                         int3 tile,
                         ...
                         bool side) :
                         m_iCurrentTurn(-2), m_bIsAttacker(false), m_cb(cb),
	                       m_army1(army1), m_army2(army2), m_tile(tile),
	                       m_hero1(hero1), m_hero2(hero2), m_side(side)
{
	const si16 MAX_ENEMY_CREATURES = 12;
...
void BattleLogic::SetCurrentTurn(ui16 turn)
{
	m_iCurrentTurn = turn;
}

void BattleLogic::MakeStatistics(ui16 currentCreatureId)
{
  // HACK: Had to use int, instead of si16, as callback doesn't know (probably)
  //       about the fact that si16 is, in fact, int...
	typedef std::map<int, CStack> map_stacks;
	map_stacks allStacks = m_cb->battleGetStacks();
	const CStack *CURRENT_STACK = m_cb->battleGetStackByID(currentCreatureId);
	if (CURRENT_STACK->position < 0) //turret
		return;
	/*
	...
	*/
	...
	for (map_stacks::const_iterator it = allStacks.begin();
	    it != allStacks.end(); ++it) {
		const CStack* STACK = &it->second;
		...
		if ((it->second.attackerOwned != 0) != m_bIsAttacker) {
			...
			// Make stats.
			int hitPoints = STACK->amount * STACK->creature->hitPoints
			                - (STACK->creature->hitPoints - STACK->firstHPleft);

			// TODO: Creature stats are 16 bit numbers, but the calculation are done
			// on 32 bits.
			m_statMaxDamage.push_back(std::pair<ui16, ui16>(
			    id, STACK->creature->damageMax * STACK->amount));
			m_statMinDamage.push_back(std::pair<ui16, ui16>(
			    id, STACK->creature->damageMin * STACK->amount));
			m_statHitPoints.push_back(std::pair<ui16, ui16>(id, hitPoints));
			m_statMaxSpeed.push_back(std::pair<ui16, ui16>(id, STACK->creature->speed));

			totalEnemyDamage += (STACK->creature->damageMax +
			                     STACK->creature->damageMin
			                     ) * STACK->amount / 2;
			totalEnemyHitPoints += hitPoints;

			// Calculate casualties.
			CreatureCasualties cs;
			// hp * amount - damage * ((att - def) >= 0)
			// hit poionts
			assert(hitPoints >= 0 &&
			       "CGeniusAI - creature cannot have hit points less than zero");

			// CGHeroInstance *attackerHero = (m_side)? m_hero1 : m_hero2;
			// CGHeroInstance *defendingHero = (m_side)? m_hero2 : m_hero1;
			
			// TODO: Do I see some more magic numbers here?
			int attackDefenseBonus = CURRENT_STACK->Attack() - STACK->Defense();
			...
			cs.damage_max = (si16)(CURRENT_STACK->creature->damageMax
			                * CURRENT_STACK->amount
			                * damageFactor);
			if (cs.damage_max > hitPoints)
				cs.damage_max = hitPoints;

			cs.damage_min = (si16)(CURRENT_STACK->creature->damageMin
			                * CURRENT_STACK->amount
			                * damageFactor);
			if (cs.damage_min > hitPoints)
				cs.damage_min = hitPoints;

			cs.amount_max = cs.damage_max / STACK->creature->hitPoints;
			cs.amount_min = cs.damage_min / STACK->creature->hitPoints;

			cs.leftHitPoints_for_max  = (hitPoints - cs.damage_max) % 
			                            STACK->creature->hitPoints;
			cs.leftHitPoint_for_min   = (hitPoints - cs.damage_min) %
			                            STACK->creature->hitPoints;

			m_statCasualties.push_back(std::pair<ui16, CreatureCasualties>(id, cs));
			
      // Enemy creatures can shoot.
			if (STACK->creature->isShooting() && STACK->shots > 0)
				m_statDistanceFromShooters.push_back( std::pair<ui16, ui16>(
				        id, m_battleHelper.GetShortestDistance(CURRENT_STACK->position,
				                                               STACK->position)) );
				                                               
			// Friendly creatures can fly or shoot - they can deal damage neglecting
			// terrain obstacles.
			if (CURRENT_STACK->hasFeatureOfType(StackFeature::FLYING) ||
			   (CURRENT_STACK->creature->isShooting() && CURRENT_STACK->shots > 0)) {
				m_statDistance.push_back( std::pair<ui16, ui16>(
				    id, m_battleHelper.GetShortestDistance(CURRENT_STACK->position,
				                                           STACK->position)) );
			} else { // We have to deal with obstacles.
				m_statDistance.push_back( std::pair<ui16, ui16>(
				    id, m_battleHelper.GetDistanceWithObstacles(CURRENT_STACK->position,
				                                            STACK->position)) );
		  }
		} else { // if ((it->second.attackerOwned != 0) != m_bIsAttacker)
			if (STACK->amount < 1)
				continue;

		  si16 hitPoints = STACK->amount * STACK->creature->hitPoints -
			                (STACK->creature->hitPoints - STACK->firstHPleft);
			totalDamage += (STACK->creature->damageMax + STACK->creature->damageMin
			                ) * STACK->amount / 2;
	...
	std::sort(m_statMaxDamage.begin(), m_statMaxDamage.end(),
		bind(&creature_stat::value_type::second, _1) >
		    bind(&creature_stat::value_type::second, _2));
	// sort min damage
	...
	std::sort(m_statCasualties.begin(), m_statCasualties.end(),
	          bind( &creature_stat_casualties::value_type::second_type::damage_max,
	              bind(&creature_stat_casualties::value_type::second, _1) ) <
		        bind( &creature_stat_casualties::value_type::second_type::damage_max,
		            bind(&creature_stat_casualties::value_type::second, _2) ));
}

BattleAction BattleLogic::MakeDecision(ui16 stackID)
{
	const CStack* currentStack = m_cb->battleGetStackByID(stackID);
	if(currentStack->position < 0) //turret
		return this->MakeDefend(stackID);
	...
	if (additionalInfo == -1 || creatures.empty())
		return MakeDefend(stackID); // Defend.
	else if (additionalInfo == -2)
		return MakeWait(stackID);

	list<ui16>::iterator it = creatures.begin();
	list<ui16>::iterator eit = creatures.end();
	for (it; it != eit; ++it) {
...
vector<ui16> BattleLogic::GetAvailableHexesForAttacker(const CStack* defender,
                                                            const CStack* attacker)
{
  typedef std::pair<int, int> hexPoint;
	...
	// TODO: should be vector<int> but for debug purpose std::pair is used
	list<hexPoint> candidates;
	vector<ui16> fields;
	
	if (defenderIsDW) {
		if (defender->attackerOwned) {
			if (!(y % 2)) { // From left side.
				// Up.
	...
	for (std::list<hexPoint>::iterator it = candidates.begin();
	    it != candidates.end(); ++it) {
		if (it->first < 1
		    || it->first > m_battleHelper.BattlefieldWidth
		    || it->second < 1
		    || it->second > m_battleHelper.BattlefieldHeight) {
		...
		if (st == NULL || st->amount < 1) {
			if (attackerIsDW) {
				...
				assert(tail_pos >= 0 && 
				    "Error during calculation position of double wide creature");
			...
			if (attacker->ID == st->ID)
				fields.push_back(new_pos);
		} // Should else () be present here.
		//++it;
	}
	return fields;
}

BattleAction BattleLogic::MakeDefend(ui16 stackID)
{
	BattleAction ba;
	ba.side = m_side;
...
BattleAction BattleLogic::MakeWait(ui16 stackID)
{
	BattleAction ba;
	ba.side = m_side;
...
BattleAction BattleLogic::MakeAttack(ui16 attackerID, ui16 destinationID)
{
	const CStack* attackerStack = m_cb->battleGetStackByID(attackerID);
	const CStack* destinationStack = m_cb->battleGetStackByID(destinationID);
	...
	if (m_cb->battleCanShoot(attackerID, m_cb->battleGetPos(destinationID))) {
		...
		ba.destinationTile = m_cb->battleGetPos(destinationID);
		return ba;
	} else {
		// go or go&attack
		int dest_tile = -1;
		vector<ui16> av_tiles = GetAvailableHexesForAttacker(
		    m_cb->battleGetStackByID(destinationID),
		    m_cb->battleGetStackByID(attackerID));
		if (av_tiles.size() < 1)
			return MakeDefend(attackerID);
		...
		for (vector<ui16>::iterator it = av_tiles.begin();
		    it != av_tiles.end(); ++it) {
			int dist =
			    m_battleHelper.GetDistanceWithObstacles(*it, m_cb->battleGetPos(attackerID));
		...
		vector<int> fields = m_cb->battleGetAvailableHexes(attackerID, false);
		...
		ba.stackNumber = attackerID;
		ba.destinationTile = static_cast<si16>(dest_tile);
		...
		if (attackerStack->hasFeatureOfType(StackFeature::DOUBLE_WIDE)) {
			...
			if (dest_tile == tail_pos) {
				...
				PrintBattleAction(ba);
#endif
        // If action is action_walk_and_attack additional info must point on 
        // enemy stack.
				assert(m_cb->battleGetStackByPos(ba.additionalInfo, false));
				// Don't attack ourselves.
				assert(m_cb->battleGetStackByPos(ba.additionalInfo, false) != attackerStack);
				return ba;
			}
		}

		for (vector<int>::const_iterator it = fields.begin();
		    it != fields.end(); ++it) {
			if (*it == dest_tile) {
				// attack!
				ba.actionType = action_walk_and_attack;
        ...
        // If action is action_walk_and_attack additional info must point on 
        // enemy stack.
				assert(m_cb->battleGetStackByPos(ba.additionalInfo));
				// Don't attack ourselves.
				assert(m_cb->battleGetStackByPos(ba.additionalInfo) != attackerStack);
				return ba;
			}
		...
		message += boost::lexical_cast<std::string>(m_battleHelper.DecodeXPosition(nearest_pos))
		        + ", Y=";
		message += boost::lexical_cast<std::string>(m_battleHelper.DecodeYPosition(nearest_pos));
		debugBox(message.c_str());

		ba.actionType = action_walk;
		ba.destinationTile = (si16)nearest_pos;
		ba.additionalInfo  = -1;
#if defined PRINT_DEBUG
		PrintBattleAction(ba);
#endif
...
list<ui16> BattleLogic::PerformBerserkAttack(ui16 stackID, si8 additionalInfo)
{
	CCreature c = m_cb->battleGetCreature(stackID);
	// attack to cause biggest damage
	list<ui16> creatures;
	...
	if (!m_statCasualties.empty()) {
		...
		for (; it != m_statCasualties.end(); ++it) {
			...
			for (creature_stat::const_iterator it2 = m_statDistance.begin();
			    it2 != m_statDistance.end(); ++it2) {
				if ((it2->first == it->first) &&
				    (it2->second - 1 <= static_cast<int>(c.speed)))
...
list<ui16> BattleLogic::PerformDefaultAction(ui16 stackID, si8 additionalInfo)
{
	// first approach based on the statistics and weights
	// if this solution was fine we would develop this idea.
	map<ui16, ui16> votes;

	for (creature_stat::iterator it = m_statMaxDamage.begin();
	     it != m_statMaxDamage.end(); ++it)
	   votes[it->first]  = 0;
	...
	if (m_statDistanceFromShooters.size())
		votes[m_statDistanceFromShooters.begin()->first] +=
		    m_battleHelper.GetVoteForDistanceFromShooters();
	...
	for (map<ui16, ui16>::iterator it = votes.begin(); it != votes.end(); ++it) {
	  // It's hostile stack
		if ((m_cb->battleGetStackByID(it->first)->attackerOwned == m_side) 
		    && (it->second > max_vote)) {
			max_vote = it->second;
			creatures.push_front(it->first);
		}
...
// For debug purposes.
void BattleLogic::PrintBattleAction(const BattleAction& action)
{
	...
	message +=
	    lexical_cast<std::string>(m_battleHelper.DecodeXPosition(action.destinationTile));
	message += ", Y = " +
	    lexical_cast<std::string>(m_battleHelper.DecodeYPosition(action.destinationTile));
	message += "\nAdditional info: ";
	
	if (action.actionType == 6) { // || action.actionType == 7)
		message += "stack - " + 
		    lexical_cast<std::string>(m_battleHelper.DecodeXPosition(action.additionalInfo));
		message += ", " +
		    lexical_cast<std::string>(m_battleHelper.DecodeYPosition(action.additionalInfo));
		message += ", creature - ";
		...
		message += lexical_cast<std::string>(action.additionalInfo);
