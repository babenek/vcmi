...
neuralNetwork::neuralNetwork(const neuralNetwork& other) : nInput(0), 
                                                           nHidden1(0), 
                                                           nHidden2(0),
                                                           nOutput(0)
{
...
void neuralNetwork::operator= (const neuralNetwork& cpy) // Assumes same structure.
{
	if (nInput != cpy.nInput ||	nHidden1 != cpy.nHidden1 ||	nHidden2 != cpy.nHidden2 ||	nOutput != cpy.nOutput)
	{
		...
		for (si16 j=0; j <= nHidden2; j++)
		  delete[] wHiddenOutput[j];
		delete[] wHiddenOutput;	

		for (si16 j=0; j <= nHidden1; j++)
	...
	for ( si16 i=0; i < nInput; i++ ) inputNeurons[i] = cpy.inputNeurons[i];
	for ( si16 i=0; i < nHidden1; i++ ) hiddenNeurons1[i] = cpy.hiddenNeurons1[i];
	for ( si16 i=0; i < nHidden2; i++ ) hiddenNeurons2[i] = cpy.hiddenNeurons2[i];
	for ( si16 i=0; i < nOutput; i++ ) outputNeurons[i] = cpy.outputNeurons[i];
...
neuralNetwork::~neuralNetwork()
{
	...
	//delete weight storage
	for (si16 i=0; i <= nInput; i++) delete[] wInputHidden[i];
	delete[] wInputHidden;

	for (si16 j=0; j <= nHidden2; j++) delete[] wHiddenOutput[j];
	delete[] wHiddenOutput;	

	for (si16 j=0; j <= nHidden1; j++) delete[] wHidden2Hidden[j];
	delete[] wHidden2Hidden;	
}
...
void neuralNetwork::feedForward(double* pattern)
{
	...
	for(si16 j=0; j < nHidden2; j++) {
		...
		hiddenNeurons2[j] = 0;
		//get weighted sum of pattern and bias neuron
		for( si16 i=0; i <= nHidden1; i++ ) hiddenNeurons2[j] += hiddenNeurons1[i] * wHidden2Hidden[i][j];
		//set to result of sigmoid
	...
	//--------------------------------------------------------------------------------------------------------
	for(si16 k=0; k < nOutput; k++)
	{
		//clear value
		outputNeurons[k] = 0;				
		
		//get weighted sum of pattern and bias neuron
		for( si16 j=0; j <= nHidden2; j++ ) outputNeurons[k] += hiddenNeurons2[j] * wHiddenOutput[j][k];
