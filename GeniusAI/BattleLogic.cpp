	...
	*/
	...
	for (map_stacks::const_iterator it = allStacks.begin(); it != allStacks.end(); ++it)
	{
		...
		if ((it->second.attackerOwned != 0) != m_bIsAttacker)
		{
			...
			m_statMaxDamage.push_back(std::pair<int, int>(
			    id, st->creature->damageMax * st->amount));
			m_statMinDamage.push_back(std::pair<int, int>(
			    id, st->creature->damageMin * st->amount));
			...
			if (st->creature->isShooting() && st->shots > 0)
				m_statDistanceFromShooters.push_back(
				    std::pair<int, int>(
				        id,
				        m_battleHelper.GetShortestDistance(currentStack->position, st->position))
				    );

			if (currentStack->hasFeatureOfType(StackFeature::FLYING)
			    || (currentStack->creature->isShooting()
			    && currentStack->shots > 0))
				m_statDistance.push_back(std::pair<int, int>(id, 
				    m_battleHelper.GetShortestDistance(currentStack->position, st->position)));
			else
				m_statDistance.push_back(std::pair<int, int>(id,
				    m_battleHelper.GetDistanceWithObstacles(currentStack->position, st->position)));
		} else {
			if (st->amount < 1)
				continue;

			int hitPoints = st->amount * st->creature->hitPoints
			                - (st->creature->hitPoints - st->firstHPleft);
			totalDamage += (st->creature->damageMax + st->creature->damageMin)
			                * st->amount / 2;
...
BattleAction CBattleLogic::MakeDecision(int stackID)
{
	...
	list<int>::iterator it = creatures.begin();
	list<int>::iterator eit = creatures.end();
	for (it; it != eit; ++it) {
...
BattleAction CBattleLogic::MakeAttack(int attackerID, int destinationID)
{
	...
	if (m_cb->battleCanShoot(attackerID, m_cb->battleGetPos(destinationID))) {
		...
		// go or go&attack
		int dest_tile = -1;
		std::vector<int> av_tiles = GetAvailableHexesForAttacker(
		    m_cb->battleGetStackByID(destinationID), m_cb->battleGetStackByID(attackerID));
		if (av_tiles.size() < 1)
			return MakeDefend(attackerID);
		...
		for (std::vector<int>::iterator it = av_tiles.begin();
		    it != av_tiles.end(); ++it) {
		...
		for (std::vector<int>::const_iterator it = fields.begin();
		    it != fields.end(); ++it) {
			if (*it == dest_tile) {
				// attack!
				ba.actionType = action_walk_and_attack;
